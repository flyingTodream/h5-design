...
export default {
    ...
    methods:{
        ...
        selected_mousedown_Handler(e){
            ...
            pos.x && (this.l = pos.x);
            pos.y && (this.t = pos.y);
            let canvasSize = document.querySelector('#js-container .editor-canvas').getBoundingClientRect();
        ...
        selectElementHandler(){
            getter.layout().elements.forEach((item, i) => {
                let item_t = item.top * this.zoom, item_r = (item.left + item.width) * this.zoom,
                    item_b = (item.top + item.height) * this.zoom, item_l = item.left * this.zoom;
                if(item_r < this.selectedArea.r && 
